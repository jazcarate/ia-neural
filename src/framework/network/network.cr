class Network
  ...
  def compute(input, results, function)
    ...
    while gen < 60000
      ...
      l0 = input
      l1 = (l0.x syn0).map(function.fn)
      l2 = (l1.x syn1).map(function.fn)
      ...
      if gen % 10000 == 0
        @log.info "Suma de errores: " + l2_error.clone.flatten.map(&.abs).sum
      end

      l2_delta = l2_error * l2.map(function.fn_d)
      ...
      l1_delta = l1_error * l1.map(function.fn_d)
